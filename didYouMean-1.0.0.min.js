...
*/
var didYouMean=(function(){var a=function(k,j,l){if(!k){return false}if(!arguments.callee.caseSensitive){k=k.toLowerCase()}var f,g=arguments.callee.threshold,c=g===null?null:g*k.length,m,d,e,h=j.length;for(e=0;e<h;e++){m=j[e];if(l){m=m[l]}if(!m){continue}if(!arguments.callee.caseSensitive){m=m.toLowerCase()}d=b(k,m);if(c===null||d<c){c=d;if(l&&arguments.callee.returnWinningObject){f=j[e]}else{f=m}}}return f||arguments.callee.nullResultValue};a.threshold=0.4;a.caseSensitive=false;a.nullResultValue=null;a.returnWinningObject=null;return a;function b(d,c){if(d.length==0){return c.length}if(c.length==0){return d.length}var e=[];var g;for(g=0;g<=c.length;g++){e[g]=[g]}var f;for(f=0;f<=d.length;f++){e[0][f]=f}for(g=1;g<=c.length;g++){for(f=1;f<=d.length;f++){if(c.charAt(g-1)==d.charAt(f-1)){e[g][f]=e[g-1][f-1]}else{e[g][f]=Math.min(e[g-1][f-1]+1,Math.min(e[g][f-1]+1,e[g-1][f]+1))}}}return e[c.length][d.length]}})();